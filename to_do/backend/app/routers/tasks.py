...
async def add_task(add_task: STaskAdd, session: DbDep, user: CUDep, cache: TCDep):
    ...
    if not check:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error with adding task"
        )
...
async def update_task(
    task_id: int, task_upd: STaskUpd, session: DbDep, user: CUDep, cache: TCDep
):
    user_id = user.id
    ...
    if not check:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error with updating task"
        )
    cached_tasks = await cache.get(user_id)
    if cached_tasks is not None:
...
async def delete_task(task_id: int, session: DbDep, user: CUDep, cache: TCDep):
    ...
    if not check:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error with deleting task"
        )
    cached_tasks = await cache.get(user_id)
    if cached_tasks is not None:
        for i in range(len(cached_tasks)):
